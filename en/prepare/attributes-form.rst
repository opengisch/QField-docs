...
external device connected via NMEA connection.
 ...
 - `@sensor_ground_speed`
   ...
   - IE
 - `@sensor_magnetic_variation`
   - The angle between the horizontal component of the magnetic field and true north, in degrees as reported by the sensor. Also known as magnetic declination. A positive value indicates a clockwise direction from true north and a negative value indicates a counter-clockwise direction.
   - 
   - IE
 - `@sensor_horizontal_accuracy`
   - The horizontal accuracy of the coordinate (in meters) as reported by the sensor.
   - IE
 - `@sensor_vertical_accuracy`
   - The vertical accuracy of the coordinate (in meters) as reported by the sensor.
   - IE
 - `@sensor_3d_accuracy`
   - The 3 dimensional accuracy of the coordinate (in meters), 3D-RMS as reported by the sensor.
   - IE
 - `@sensor_vertical_speed`
   - The vertical speed (in m/s) as reported by the sensor.
   - IE 
 - `@sensor_source_name`
   ...
   - IE
 - `@sensor_pdop`
   - Position dilution of precision as reported by the sensor.
   - E
 - `@sensor_hdop`
   - Horizontal dilution of precision as reported by the sensor.
   - E
 - `@sensor_vdop`
   - Vertical dilution of precision as reported by the sensor.
   - E
 - `@sensor_number_of_used_satellites`
   - Number of satellites as reported by the sensor.
   - E
 - `@sensor_used_satellites`
   - A list of satellites in use (pri) as reported by the sensor.
   - `array_count(@position_used_satellites)`
   - E
 - `@sensor_quality_description`
   - A human readable and translated string for the quality as reported by the sensor. E.g. "Fixed RTK"
   - E
 - `@sensor_fix_status_description`
   - The GPS Fix Status "NoData", "NoFix", "Fix2D" or "Fix3D" as reported by the sensor.
   - E
 - `@sensor_fix_mode`
   - Fix mode (where 'M' = Manual, forced to operate in 2D or 3D or 'A' = Automatic, 3D/2D) as reported by the sensor.
   - E
